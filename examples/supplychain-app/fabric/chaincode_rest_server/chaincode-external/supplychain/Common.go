...
func (s *SmartContract) updateState(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	var request common.UpdateRequest
	...
	var productData common.Product
	...
	if err != nil {
		...
		var containerData common.Container
...
func (s *SmartContract) getHistory(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	var historyList []common.History
	buffer.WriteString("[")
	for iterator.HasNext() {
		...
		var historyItem common.History
...
func isInHistory(list []common.History, item common.History) bool {
	for _, v := range list {
		if v == item {
			return true
		}
